      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#111110] flex items-center gap-2">
            <Syringe size={24} className="text-[#1A6B5E]" />
            Sanitário
          </h1>
          <p className="text-[#6B6B65] text-sm mt-1">Controle de vacinas e medicamentos</p>
        </div>
        <button
          onClick={() => setDrawerOpen(true)}
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#1A6B5E] hover:bg-[#155A4F] text-white text-sm font-semibold transition"
        >
          <Plus size={16} />
          Registro Individual
        </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 bg-[#F5F4EF] border border-[#E8E8E3] rounded-lg text-xs text-[#111110] placeholder:text-[#A8A8A2] focus:outline-none focus:ring-1 focus:ring-[#2F6A47] focus:border-[#2F6A47]"
            />
          </div>
          <select
            value={filters.tipo}
            onChange={(e) => handleFilterChange('tipo', e.target.value)}
            className="w-full bg-[#F5F4EF] border border-[#E8E8E3] rounded-lg px-3 py-2 text-xs text-[#111110] focus:outline-none focus:ring-1 focus:ring-[#2F6A47] focus:border-[#2F6A47]"
          >
          ...
          <DatePickerBR
            ...
            className="w-full bg-[#F5F4EF] border border-[#E8E8E3] rounded-lg px-3 py-2 text-xs text-[#111110] placeholder:text-[#A8A8A2] focus:outline-none focus:ring-1 focus:ring-[#2F6A47] focus:border-[#2F6A47]"
          />
          <DatePickerBR
      ...
      <div className="bg-white rounded-xl border border-[#E8E8E3] shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <Syringe size={48} className="text-[#E8E8E3] mx-auto mb-3" />
            <p className="text-[#6B6B65] font-medium">Nenhum registro encontrado</p>
            <p className="text-[#A8A8A2] text-sm mt-1">Adicione registros sanitários usando o botão acima</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#F0EFEB]">
                {registros.map((r) => (
                  <tr key={r.id} className="hover:bg-[#F5F4EF] transition-colors">
                    <td className="px-4 py-3 font-medium text-[#111110]">
                      {r.animal.numero ? `Nº ${r.animal.numero}` : `#${r.animal.id}`}
                      <span className="ml-1.5 text-xs text-[#A8A8A2]">{r.animal.denominacao}</span>
                    </td>
                    <td className="px-4 py-3 text-[#6B6B65]">{r.animal.proprietario.name}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${r.tipo === 'VACINA' ? 'bg-[#EEF3FB] text-[#1B58A3]' : 'bg-[#FBF3EC] text-[#8A4E1C]'}`}>
                        {r.tipo === 'VACINA' ? 'Vacina' : 'Medicamento'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-[#111110] font-medium">{r.produto}</td>
                    <td className="px-4 py-3 text-[#6B6B65]">{formatDateBR(r.data)}</td>
                    <td className="px-4 py-3 text-[#6B6B65]">{r.dose ?? '—'}</td>
                    <td className="px-4 py-3 text-[#A8A8A2] max-w-xs truncate">{r.observacoes ?? '—'}</td>
                    <td className="px-4 py-3">
                      <button
